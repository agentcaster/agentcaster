...
#       print bg_rgb, self.fg_color.getRgb()
    ...
    def getPlotTitle(self, prof_coll):
        ...
        lat = prof_coll.getHighlightedProf().latitude if hasattr(prof_coll.getHighlightedProf(), 'latitude') else None
        lon = prof_coll.getHighlightedProf().longitude if hasattr(prof_coll.getHighlightedProf(), 'longitude') else None
        
        # Add coordinates to the title if available
        coord_str = ""
        if lat is not None and lon is not None:
            coord_str = f" [{lat:.2f},{lon:.2f}]"
        
        plot_title = loc + coord_str + '   ' + datetime.strftime(date, "%Y%m%d/%H%M")
        if model == "Archive":
            fhour_str = ""

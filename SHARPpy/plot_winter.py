"""
Plot a Winter Weather Parameters display using SHARPpy's native Qt visualization
"""
import os
import sys
import numpy as np
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('SHARPpy Winter Weather Parameters')
        self.resize(800, 600)
        ...
        # Create the Winter Weather Parameters display
        self.winter = plotWinter()
        
        # Set the profile
        if self.prof_collection.hasCurrentProf():
            prof = self.prof_collection.getHighlightedProf()
            self.winter.setProf(prof)

        # Set the widget as the central widget
        self.setCentralWidget(self.winter)

"""
Plot a Storm Slinky display using SHARPpy's native Qt visualization
"""
import os
import sys
import numpy as np
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('SHARPpy Storm Slinky')
        self.resize(800, 600)
        ...
        # Create the Storm Slinky display
        self.slinky = plotSlinky()
        
        # Set the profile and parcel
        if self.prof_collection.hasCurrentProf():
            prof = self.prof_collection.getHighlightedProf()
            self.slinky.setProf(prof)
            
            # Create and set a surface-based parcel
            pcl = params.parcelx(prof, flag=1)  # flag=1 for surface-based parcel
            self.slinky.setParcel(pcl)

        # Set the widget as the central widget
        self.setCentralWidget(self.slinky)

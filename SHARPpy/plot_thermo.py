"""
Plot a thermodynamic text display using SHARPpy's native Qt visualization
"""
import os
import sys
import numpy as np
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('SHARPpy Thermodynamic Parameters')
        ...
        # Create the thermodynamic text display
        # Default parcel types to display
        pcl_types = ["SFC", "ML", "MU", "FCST"]
        self.thermo = plotText(pcl_types)
        
        # Set the profile
        if self.prof_collection.hasCurrentProf():
            prof = self.prof_collection.getHighlightedProf()
            self.thermo.setProf(prof)

        # Set the widget as the central widget
        self.setCentralWidget(self.thermo)

"""
Plot a Temperature Advection display using SHARPpy's native Qt visualization
"""
import os
import sys
import numpy as np
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('SHARPpy Temperature Advection')
        self.resize(800, 600)
        ...
        # Create the Temperature Advection display
        self.advection = plotAdvection()
        
        # Set the profile
        if self.prof_collection.hasCurrentProf():
            prof = self.prof_collection.getHighlightedProf()
            self.advection.setProf(prof)

        # Set the widget as the central widget
        self.setCentralWidget(self.advection)
